...
def data_gen(V, batch, nbatches):
    ...
    for i in range(nbatches):
        data1 = torch.from_numpy(X0.reshape(394,8))#.long()
        data1[:, 0] = 1
        data2 = torch.from_numpy(Y0.reshape(394,8))#.long()
        data2[:, 0] = 1
...
class SimpleLossCompute:
    ...
    def __call__(self, x, y, norm):
        x = self.generator(x)
        x=torch.sum(x.reshape(394,7,-1), (2))
...
### import data and make the transformation
series = read_csv('ca_ili.csv')
amplitude   = series["ili_rate"].values
dataset = np.array(amplitude.astype('float32')).reshape(-1,1)
dataset 
...
# save the trained parameters and model in a pth. file
# PATH = './pytorch_time_series_model_loss_ili_rate_2.pth'
# torch.save(model.state_dict(), PATH)

### take a glance of the training data prediction at place 300
PATH = './pytorch_time_series_model_loss_ili_rate_2.pth'
X0=trainX[0:-2]
Y0=trainX[1:-1]
...
### make the prediction and collect all prediction to a list
res=[]
actual = []
prediction = []
for place in range(0,30):
    ...
    actual.append(Y0[0][0][-1])
    prediction.append(pred.detach().numpy())

### transfer inverse to the original data and calculate the rmse 
ts_data  = series["ili_rate"].values
dataset = np.array(ts_data.astype('float32')).reshape(-1,1)

scaler = MinMaxScaler(feature_range=(0, 1)) 
dataset = scaler.fit_transform(dataset)

np_pred = np.asarray(prediction)
np_pred = np_pred.reshape(-1, 1)
np_act = np.asarray(actual)
np_act =np_act.reshape(-1, 1)

orig_pred = scaler.inverse_transform(np_pred)
orig_act  = scaler.inverse_transform(np_act)

from sklearn.metrics import mean_squared_error
math.sqrt(mean_squared_error(orig_pred, orig_act))

### plot the actual data and prediction data
plt.figure(figsize=(12,4))
plt.title("CA ILI Rate Prediction")
plt.ylabel("ILI Rate")
plt.grid(True)
plt.plot(orig_act )
# plt.plot(trainPredictPlot)
plt.plot(orig_pred,color= "red")
plt.show()
